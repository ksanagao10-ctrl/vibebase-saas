...
入口 `/#calculator/channels`。`/api/relay-catalog` 读取公开报价 API 的模型目录，`/api/relay-costs` 将精确型号、分组、上下文阶梯报价代入统一任务量。首批使用 priceCatalog.sources 的前 20 个独立站点；2026-09-22 本地真实访问均返回 JSON 模型目录。公开目录不代表账号可调用。

前台按每批 5 家、4 批并行查询，共用既有 5 分钟缓存和失败回退；过期报价禁止成本计算。模型选项仅使用目录实际出现且已映射的版本。所有原始模型 ID 可在渠道目录中搜索。未映射的型号不猜测版本，音视频等计费规格未核实时只展示基础报价。

文本任务基础费用 = 对话数 × 每次模型请求轮数 ×（每轮输入 × 输入百万单价 + 每轮输出 × 输出百万单价）/ 1e6；按次收费则按请求数计算。失败扣费沿用交付预算的统计公式。站内额度不得直接与现金 USD 排名。每家单独填写充值实付/所得额度、余额、附加费、其他人民币费用；这些数据仅保存于本机。人民币排序只对已补全换算条件的报价生效，未知项排在末尾。分组默认首项，须核实账号资格，可手动切换。

import {radarHome} from './radar.js';
import {experienceDetails,experienceFeedback,bindExperience} from './experience.js';
...
function modelCard(m){return '<article class="discovery-card"><div class="discovery-card-top"><span class="eyebrow">MODEL GUIDE</span><span class="tag">'+esc(m.status||(m.guideCount?'任务候选':'目录备选'))+'</span></div><h3>'+esc(m.name)+'</h3><p>'+esc(m.desc)+'</p><div class="tag-row">'+m.tasks.map(t=>'<span class="tag">'+esc(task(t).name)+'</span>').join('')+'</div><div class="discovery-card-foot">'+link('model/'+m.id,'查看模型与渠道 →')+'</div></article>';}
export function taskMatches(category='all'){return d.taskGuides.filter(g=>category==='all'||g.category===category);}
function taskGuideCard(g){
 return '<a class="discovery-card task-guide-card" href="#models/'+g.category+'/'+g.id+'" data-route="models/'+g.category+'/'+g.id+'"><span class="eyebrow">'+esc(task(g.category).name)+' · '+g.picks.length+' 个针对性候选</span><h3>'+esc(g.name)+'</h3><p>'+esc(g.goal)+'</p><p class="mini-meta">'+g.picks.map(p=>esc((model(p.model).vendor==='OpenAI'?'OpenAI · ':'')+model(p.model).name)).join(' / ')+'</p><span class="text-link">查看匹配理由与用法 →</span></a>';
}
function taskPickCard(g,p){
 const m=model(p.model);
 return '<article class="discovery-card task-pick"><span class="eyebrow">'+esc(p.role)+'</span>'+(m.vendor?'<span class="tag">'+esc(m.vendor)+'</span>':'')+'<h2>'+esc(m.name)+'</h2><span class="tag">'+esc(m.status||'资料已核对')+'</span><h3>为什么匹配这个任务</h3><p>'+esc(p.why)+'</p><h3>怎样使用</h3><p>'+esc(p.setup)+'</p><p class="model-id">'+esc(m.modelId)+'</p><div class="discovery-card-foot">'+link('model/'+m.id,'型号与接入详情 →')+' · '+sourceLink(m.source,'能力依据 ↗')+'</div></article>';
}
function taskDirectory(category='all',guideId){
 if(category==='openai')return header('OPENAI · GPT','OpenAI（ChatGPT 开发商）模型','按写作、编程、图像、语音与知识库任务匹配具体 API 型号。')+'<div class="notice"><p>'+esc(d.openaiReview.note)+'</p>'+sourceLink(d.openaiReview.source,'查看官方型号目录 ↗')+'</div>'+link('models','← 返回全部任务')+'<div class="discovery-grid three">'+d.models.filter(m=>m.vendor==='OpenAI'&&m.guideCount).map(modelCard).join('')+'</div><div class="notice"><h2>视频与重排为什么不硬凑</h2>'+d.openaiReview.exclusions.map(x=>'<p>'+esc(x.reason)+'</p>').join('')+'</div>';

 if(category==='catalog')return header('MODEL CATALOG','全部型号资料','保留历史版本与免费实验型号，便于已接入项目查阅；新任务优先使用任务选型。')+link('models','← 按具体任务选择')+'<div class="discovery-grid three">'+[...d.models].sort((a,b)=>(b.guideCount||0)-(a.guideCount||0)).map(modelCard).join('')+'</div>';
 if(!d.tasks.some(t=>t.id===category))category='all';
 const rows=taskMatches(category),g=rows.find(g=>g.id===guideId);
 const intro=header('MATCH THE JOB','按具体任务，找到合适模型','6 大类 · 21 个具体任务。按输入、交付物与能力匹配，主力、专项和节省成本的备选分别说明。')+tabs([['all','全部任务'],...d.tasks.map(t=>[t.id,t.name])],category,'models/')+'<div class="tool-links">'+link('models/openai','OpenAI / GPT 系列（ChatGPT 开发商）','tool-link')+'</div>';
 if(!g)return intro+'<div class="discovery-grid three">'+rows.map(taskGuideCard).join('')+'</div><div class="notice"><p>选型依据官方能力文档与渠道目录；推荐理由是编辑判断，未进行本站统一效果实测。免费与价格最低不等于适合当前任务。</p>'+link('models/catalog','查阅全部型号（含历史与免费备选） →')+' · '+link('boards/free','查看免费模型条件 →')+'</div>';
 return intro+'<nav class="discovery-tabs" aria-label="具体任务">'+rows.map(x=>link('models/'+category+'/'+x.id,x.name,x.id===g.id?'task-choice active':'task-choice')).join('')+'</nav><section class="notice"><span class="eyebrow">你要完成的任务</span><h2>'+esc(g.name)+'</h2><p>'+esc(g.goal)+'</p><p><strong>选择与验收：</strong>'+esc(g.criteria)+'</p></section><div class="discovery-grid '+(g.picks.length>=3?'three':'two')+' task-picks">'+g.picks.map(p=>taskPickCard(g,p)).join('')+'</div><section class="notice"><h3>哪些情况不要这样选</h3><p>'+esc(g.avoid)+'</p><p class="mini-meta">2026-09-21 核对型号与能力来源；这是任务适配建议，不是付费调用实测排名。最终效果用自己的样本比较。</p></section><div class="tool-links">'+link('models/'+category,'查看本类其他任务','tool-link')+((g.picks.some(p=>d.quotes.some(q=>q.model===p.model)||d.mediaQuotes.some(q=>q.model===p.model)))?link('boards/'+category,'查看已收录的成本样本','tool-link'):'')+'</div>';
}
...
function articleCard(a,featured=false){return '<article class="story-card '+(featured?'story-featured':'')+'"><span class="eyebrow">'+esc(a.category)+' · 文档核对 · '+esc(a.difficulty)+'</span><h3>'+link('read/'+a.id,a.title,'story-title')+'</h3><p>'+esc(a.summary)+'</p><div class="story-meta"><span>'+esc(a.author)+'</span><span>约 '+a.minutes+' 分钟 · '+a.date+'</span></div>'+link('read/'+a.id,'阅读全文 ↗')+'</article>';}

const industryModel=id=>d.industryModels.find(m=>m.id===id);
export function industryPlanCost(plan,tasks=d.industryPricing.tasks){
 const p=d.industryPricing;
 return plan.recipe.reduce((total,r)=>{const m=industryModel(r.model);return total+tasks*r.share*(p.inputTokens*m.input+p.outputTokens*m.output)/1e6;},0);
}
...
function boardPreview(tab='price'){
 ...
 if(tab==='picks')return '<div class="discovery-grid three">'+d.taskGuides.filter(g=>['copy','build','product-edit'].includes(g.id)).map(taskGuideCard).join('')+'</div>';
 return quoteTable('flash',1,.2,true)+'<p class="mini-meta">同模型标准文本报价；聚合渠道为目录起价。按输入 100 万、输出 20 万 Token 估算；同价不分高低，附加费用另计。</p>';
}
export function homeDiscovery(){
 return radarHome()+section('按交付算成本，用证据选渠道','把预算、渠道记录与实际使用经验连起来。','calculator','<div class="tool-links">'+link('calculator','交付成本计算 →','tool-link')+link('probes','中转站实测档案 →','tool-link')+link('covibe','CoVibe 经验库 →','tool-link')+'</div>')+section('先选任务，再选模型','从你的实际需求出发，找到模型、接入方式和上手路径。','models','<div class="task-grid">'+d.tasks.map(t=>'<a href="#models/'+t.id+'" data-route="models/'+t.id+'" class="task-card"><span class="task-symbol" aria-hidden="true">'+esc(t.icon)+'</span><h3>'+esc(t.name)+'</h3><p>'+esc(taskMatches(t.id).map(g=>g.name).join(' / '))+'</p><span class="task-arrow" aria-hidden="true">↗</span></a>').join('')+'</div>')+
...
export function searchDiscovery(query,providers=[]){
 const q=query.trim().toLowerCase();
 return {models:q?d.models.filter(m=>(m.name+' '+m.desc+' '+(m.vendor==='OpenAI'?'OpenAI ChatGPT GPT ':'')+' '+m.tasks.map(t=>task(t).name).join(' ')).toLowerCase().includes(q)):[],
...
export function renderDiscovery(route,providers=[]){
 ...
 const cat=arg?decodeSafe(arg):'全部';body=header('LEARN · BUILD · SHARE','CoVibe','一起选好模型，把 AI 真正用起来。')+link('radar','从最新动态整理攻略提纲 →','tool-link')+tabs(['全部',...new Set(d.articles.map(a=>a.category))].map(x=>[encodeURIComponent(x),x]),encodeURIComponent(cat),'covibe/')+'<div class="discovery-grid two">'+d.articles.filter(a=>cat==='全部'||a.category===cat).map(a=>articleCard(a)).join('')+'</div>';
 }else if(page==='read'){
 const a=d.articles.find(x=>x.id===arg);
 body=a?'<article class="reading-page">'+link('covibe','← 返回 CoVibe')+header(a.category,a.title,a.summary)+'<div class="reading-meta">'+esc(a.author)+' · '+a.date+' 更新 · '+a.difficulty+' · 约 '+a.minutes+' 分钟 <button class="small-action" id="shareArticle">复制文章链接</button><span id="shareStatus" role="status"></span></div><p class="mini-meta">'+esc(a.evidence)+'</p>'+experienceDetails(a.id)+'<nav class="reading-toc" aria-label="文章目录">'+a.sections.map(([h],i)=>'<button data-reading-section="'+i+'">'+esc(h)+'</button>').join('')+'</nav>'+a.sections.map(([h,p],i)=>'<section id="reading-'+i+'" class="reading-section"><h2>'+esc(h)+'</h2><p>'+esc(p)+'</p></section>').join('')+'<section class="reading-section"><h2>参考资料</h2><ul>'+a.sources.map(([name,url])=>'<li>'+sourceLink(url,name)+'</li>').join('')+'</ul></section><section class="reading-section"><h2>继续探索</h2><div class="tool-links">'+a.models.map(id=>link('model/'+id,model(id).name,'tool-link')).join('')+[...new Set(a.models.map(id=>model(id).provider).filter(Boolean))].map(id=>'<button class="tool-link" data-discover-provider="'+id+'">'+(id==='official-gemini'?'Google 官方接入':'OpenRouter 接入')+' →</button>').join('')+'</div>'+link('covibe','阅读更多经验与教程 →')+'</section>'+experienceFeedback(a.id)+'</article>':header('NOT FOUND','文章未找到','请返回 CoVibe 选择文章。')+link('covibe','返回 CoVibe');
 }else if(page==='search'){
...
export function bindDiscovery({navigate,openProvider,estimate}){
 ...
 document.querySelector('#industrySearch')?.addEventListener('submit',e=>{e.preventDefault();navigate('industries/'+encodeURIComponent(e.currentTarget.dataset.group)+'/'+encodeURIComponent(document.querySelector('#industryQuery').value.trim()));});

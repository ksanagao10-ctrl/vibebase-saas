...
export function homeDiscovery(){
 return section('按交付算成本，用证据选渠道','把预算、渠道记录与实际使用经验连起来。','calculator','<div class="tool-links">'+link('calculator','交付成本计算 →','tool-link')+link('probes','中转站实测档案 →','tool-link')+link('covibe','CoVibe 经验库 →','tool-link')+'</div>')+section('先选任务，再选模型','从你的实际需求出发，找到模型、接入方式和上手路径。','models','<div class="task-grid">'+d.tasks.map(t=>'<a href="#models/'+t.id+'" data-route="models/'+t.id+'" class="task-card"><span class="task-symbol" aria-hidden="true">'+esc(t.icon)+'</span><h3>'+esc(t.name)+'</h3><p>'+esc(taskMatches(t.id).map(g=>g.name).join(' / '))+'</p><span class="task-arrow" aria-hidden="true">↗</span></a>').join('')+'</div>')+

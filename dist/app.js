import {operationsPage,bindOperations} from './operations.js';
import {radarPage,radarSourcesPage,bindRadar} from './radar.js';
...
function affiliatePage(){
  const relayTotal=affiliatePrograms.filter(r=>r.scope==='relay').length;
  const officialTotal=affiliatePrograms.filter(r=>r.scope==='official').length;
  ...
  return `<div class="page"><section class="affiliate-hero"><div class="aff-intro"><div class="eyebrow"><span class="eyebrow-dot"></span>VIBEBASE ALLIANCE</div><h1>发现优质平台，<br>连接合作机会。</h1><p>汇集官方大模型与中转服务的合作入口。平台先行收录，合作细节后续商务对接。</p><div class="aff-flow"><span class="flow-step">浏览平台</span><span class="flow-arrow">→</span><span class="flow-step">前往官方入口</span><span class="flow-arrow">→</span><span class="flow-step">洽谈合作</span></div></div><div class="aff-widget"><div class="wallet-label">公开页面核验记录</div><div class="wallet-number">${affiliatePrograms.length}<small> 个平台入口</small></div><div class="aff-review-stats"><div><b>${relayTotal}</b><span>全目录中转站线索</span></div><div><b>${officialTotal}</b><span>已收录的官方入口</span></div><div><b>${opportunities}</b><span>有公开奖励说明</span></div></div><p class="aff-review-date">核查日期 ${affiliateCheckedAt} · 规则可能随时调整</p></div></section>
  <section class="section"><div class="section-head"><div><h2>平台与合作入口</h2><p>全部 ${affiliatePrograms.length} 个平台入口均已收录。已取得的专属链接直接接入，其余提供平台或目录入口，合作资格与结算条件后续商务对接。</p></div></div>
  <div class="aff-controls"><label>搜索平台<input id="affiliateQ" type="search" placeholder="例如：MiMo、阿里云、Volt" value="${esc(state.affiliateQuery)}"></label><label>平台范围<select id="affiliateScope"><option value="all" ${state.affiliateScope==='all'?'selected':''}>全部 ${affiliatePrograms.length} 个入口</option><option value="official" ${state.affiliateScope==='official'?'selected':''}>${officialTotal} 个官方入口</option><option value="relay" ${state.affiliateScope==='relay'?'selected':''}>${relayTotal} 个中转站</option></select></label><label>奖励类型<select id="affiliateKind">${[['all','全部类型'],['cash','可提现 / 多类奖励'],['credits','站内额度'],['enterprise','企业合作'],['unknown','待核验']].map(([v,t])=>`<option value="${v}" ${state.affiliateKind===v?'selected':''}>${t}</option>`).join('')}</select></label></div>
  <p id="affiliateCount" class="aff-count" aria-live="polite"></p><div class="aff-program-grid">${rows.map(r=>{const ref=activeReferral(r);return `<article class="aff-program" data-aff-scope="${r.scope}" data-aff-kind="${r.kind}"><div class="aff-program-head"><span class="eyebrow">${r.scope==='official'?'官方平台':`中转站 #${r.rank}`}</span><span class="aff-status ${r.status==='documented'?'ok':'warn'}">${esc(affiliateStatusLabels[r.status])}</span></div><h3>${esc(r.name)}</h3><p class="aff-type">${esc(affiliateKindLabels[r.kind])}</p><p class="aff-reward">${esc(r.reward)}</p><p class="aff-note">${esc(r.note)}</p><div class="aff-program-links"><a href="${esc(safeUrl(ref||r.entryUrl))}" target="_blank" rel="${ref?'sponsored ':''}noopener noreferrer">${ref?'通过推广链接访问 ↗':'查看平台入口 ↗'}</a><a href="${esc(safeUrl(r.sourceUrl))}" target="_blank" rel="noopener noreferrer">核查来源</a></div>${ref?'<p class="aff-disclosure">推广链接：符合平台条件时，VibeBase 可能获得奖励。</p>':''}</article>`}).join('')}</div><p id="affiliateEmpty" class="notice" hidden>没有匹配的平台，请调整搜索或筛选条件。</p></section>
  <section class="section"><div class="notice"><strong>如何理解这些结果</strong><br>站内额度和赠金通常有使用范围与有效期，不应当作现金。企业合作通常需要主体资质、审核和另行协商。需登录的计划尚未核实比例；访问失败和未确认公开计划也不等于平台没有计划。收录代表提供平台与合作线索，不代表已经建立合作关系。相关条件作为备注保留，不影响平台展示。</div></section></div>`;
}
...
function render(){
  ...
  app.innerHTML= route==='operations'?operationsPage():route==='radar'?radarPage():route==='radar/sources'?radarSourcesPage():route==='home'?home():route==='explore'?explore():route==='apps'?appsPage():route==='compare'?comparePage():route==='calculator/channels'?relayComparePage():route==='calculator'?deliveryPage():route==='calculator/token'?calculatorPage():route==='probes'?probesPage():route==='affiliate'?affiliatePage():(renderDiscovery(route,providers)||home());
...
function bind(){
  bindOperations();bindRadar();bindDelivery();bindProbes();bindRelayCompare();
